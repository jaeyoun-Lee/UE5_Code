...
DECLARE_MULTICAST_DELEGATE(FOnAttckHitCheckDelegate);		// 공격이 성공했는지 확인하는 델리게이트
DECLARE_MULTICAST_DELEGATE(FOnResetCheckDelegate);			// 공격을 초기화하는 델리게이트
DECLARE_MULTICAST_DELEGATE(FOnEnemyDieDelegate);			// Enemy가 죽으면 호출되는 델리게이트
...
public:
	...
	FOnResetCheckDelegate OnResetCheck;			// 공격이 성공했는지 확인하는 델리게이트 변수
	FOnAttckHitCheckDelegate OnHitCheck;		// 공격을 초기화하는 델리게이트 변수
	FOnEnemyDieDelegate OnEnemyDie;				// Enemy가 죽으면 호출되는 델리게이트 변수
